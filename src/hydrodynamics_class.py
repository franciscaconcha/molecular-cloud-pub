...
class Hydro:
        def __init__(self, hydro_code, particles):
                ...
                self.sink_attributes = ['name', 'birth_age', 'angular_momentum', 'mass', 'radius', 'x', 'y', 'z', 'vx', 'vy', 'vz', 'Lx', 'Ly', 'Lz']
        ...
        def evolve_model(self, model_time):
            ...
            print "final N=", len(self.sink_particles),len(self.code.dm_particles)
            if len(self.sink_particles)>0:
                    print "mean sink mass:", self.sink_particles.mass.min().in_(units.MSun), self.sink_particles.mass.mean().in_(units.MSun), self.sink_particles.mass.max().in_(units.MSun)
        ...
        def merge_sinks(self):
            ...
            for cc in ccs:
                ...
                merge_two_sinks(self.sink_particles, cc.copy())
                self.sink_particles.synchronize_to(self.code.dm_particles)
                print "sinks merged"

def merge_two_sinks(bodies, particles_in_encounter):
