        ...
        if len(hydro.sink_particles) > 0:
            ...
            for sink in hydro.sink_particles:
                if sink.mass > mass_treshold_for_star_formation:
                    ...
                    stars_from_sink = new_fractal_cluster_model(N=len(masses),
                                                                fractal_dimension=1.6,
                                                                convert_nbody=local_converter,
                                                                virial_ratio=0.5)
                    print sink.vx.value_in(pc/Myr), sink.vy.value_in(pc/Myr), sink.vz.value_in(pc/Myr)
                    print numpy.mean(stars_from_sink.vx.value_in(pc/Myr)), numpy.mean(stars_from_sink.vy.value_in(pc/Myr)), numpy.mean(stars_from_sink.vz.value_in(pc/Myr))
